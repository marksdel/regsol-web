		...
		if (!settings.customDialogSelector) {
			...
			if(!settings.message) {
				if(settings.explicitConsent) {
					...
					settings.message = 'This website uses cookies to ensure users get the best experience possible.';
				}
			}
			...
			if(settings.explicitConsent) {
				...
				html += '<div style="margin-top:5px;"><input type="submit" name="impliedsubmit" id="impliedsubmit" class="btn btn-primary" value="OK"/></div></div>';
			}		
			html += '</div></div></div>';
			$('body').append(html);
